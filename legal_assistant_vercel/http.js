...
// دالة مساعدة: توليد نسخة معكوسة للبحث في PDF المقلوب
export function reverseNormalizeArabic(input) {
  const normalized = normalizeArabic(input);
  // عكس كل كلمة منفردة
  const wordReversed = normalized.split(' ').map(w => [...w].reverse().join('')).join(' ');
  // عكس النص كله
  const fullReversed = [...normalized].reverse().join('');
  return { wordReversed, fullReversed, normalized };
}
