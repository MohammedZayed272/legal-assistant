...
// توليد كل الأشكال المحتملة للنص العربي المقلوب للبحث
function generateArabicVariants(text) {
  if (!text) return [];
  const normalized = normalizeArabic(text);
  const variants = new Set([normalized]);
  // عكس كل الكلمات
  variants.add(normalizeArabic(reverseArabicWords(text)));
  // عكس النص بالكامل
  variants.add(normalizeArabic([...text].reverse().join('')));
  // عكس كل كلمة منفردة
  const wordReversed = normalized.split(' ').map(w => [...w].reverse().join('')).join(' ');
  variants.add(wordReversed);
...
function scoreCandidate(candidate, keywords, normalizedQuestion, compactQuestion) {
  ...
  for (const keyword of keywords) {
    ...
    // بحث في النص المقلوب (مشكلة PDF العربي)
    const variants = generateArabicVariants(keyword);
    for (const variant of variants) {
      if (variant !== keyword && normalized.includes(variant)) {
        score += keyword.length >= 5 ? 2 : 1;
      }
      const variantCompact = variant.replace(/\s+/g, '');
      if (variantCompact.length >= 4 && compactText.includes(variantCompact)) {
        score += 2;
      }
    }
  }
  ...
  for (let i = 0; i < questionWords.length - 1; i++) {
    ...
    // بحث في النص المعكوس
    const reversedPhrase = normalizeArabic([...phrase].reverse().join(''));
    if (reversedPhrase !== phrase && normalized.includes(reversedPhrase)) score += 3;
  }

  if (compactQuestion.length >= 10 && compactText.includes(compactQuestion.slice(0, Math.min(36, compactQuestion.length)))) {
    score += 10;
  }
  // بحث بالنص المعكوس للسؤال كاملاً
  const reversedCompact = normalizeArabic([...compactQuestion].reverse().join('').replace(/\s+/g, ''));
  if (reversedCompact.length >= 8 && compactText.includes(reversedCompact.slice(0, Math.min(30, reversedCompact.length)))) {
    score += 6;
  }
...
7. تنبيه مهم جداً عن ملفات PDF العربية: بعض الكلمات في المقتطفات قد تظهر مقلوبة الحروف بسبب طريقة تخزين PDF مثل "ةداملا" بدلاً من "المادة" أو "نوناقلا" بدلاً من "القانون". يجب عليك:
   - قراءة هذه الكلمات المقلوبة وفهمها من سياقها
   - معاملتها كأنها مكتوبة بشكل صحيح
   - الإجابة على السؤال إذا وجدت المعلومة سواء كانت مقلوبة أو سليمة
   - لا تقل "لا توجد معلومات" فقط لأن النص يبدو غير مقروء — حاول قراءته معكوساً
