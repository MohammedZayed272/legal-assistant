...
## تحديث مهم لمعالجة ملفات PDF العربية

تمت إضافة تحسينات لاستخراج وقراءة ملفات PDF العربية، خصوصاً ملفات الأنظمة التي يظهر فيها النص أحياناً مقلوباً مثل `ةداملا` بدلاً من `المادة`.

بعد نشر هذه النسخة، يُفضّل حذف ملف PDF القديم من واجهة الإدارة ثم رفعه مرة أخرى حتى يتم حفظ النص بالاستخراج المحسّن. إذا لم تحذف الملف القديم، ستستفيد من تحسينات البحث الجديدة جزئياً، لكن أفضل نتيجة تكون بعد إعادة رفع الملف.

متغيرات اختيارية لضبط البحث:
